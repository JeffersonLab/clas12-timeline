...
/**
 * Produces the timeline of `mean` and `width` of residual_LR per layer.
    ...
    (1..8).collect{layer_number->
      ...
      if (h_integrated!=null) {
        if (h_integrated.getBinContent(h_integrated.getMaximumBin()) > 20 && integrated_entries>100){ //`good_statistics`
          ...
          data[run].put(String.format("fit_ahdc_residual_LR_layer%d", layer_number), f_integrated)
          data[run].put(String.format("mean_ahdc_residual_LR_layer%d", layer_number), -2.0 + h_integrated_mean) //offset
          data[run].put(String.format("width_ahdc_residual_LR_layer%d", layer_number), -2.0 + h_integrated_rms) //offset
          has_data.set(true)
        }
      }
    }
