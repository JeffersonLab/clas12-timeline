...
class alert_ahdc_residual_LR_layer {
    ...
    (1..8).collect{layer_number->
      ...
      if (h_integrated!=null) {
        if (h_integrated.getBinContent(h_integrated.getMaximumBin()) > 20 && h_integrated.getEntries()>100){
          ...
          data[run].put(String.format("fit_ahdc_residual_LR_layer%d", layer_number), f_integrated)
          data[run].put(String.format("mean_ahdc_residual_LR_layer%d", layer_number),  -2 + h_integrated_mean)//offset to help identify the poor statistics
          data[run].put(String.format("width_ahdc_residual_LR_layer%d", layer_number), -2 + h_integrated_rms)//offset to help identify the poor statistics
          has_data.set(true)
        }
      }
    }
  ...
  def write() {
    ...
    ['mean', 'width'].each{variable->
      ...
      out.mkdir('/timelines')
      (1..8).collect{layer_number->
        def name = String.format('ahdc_residual_LR_layer%d', layer_number)
      ...
      out.writeFile(String.format('alert_ahdc_residual_LR_%s.hipo', variable))
    }
  }
}
