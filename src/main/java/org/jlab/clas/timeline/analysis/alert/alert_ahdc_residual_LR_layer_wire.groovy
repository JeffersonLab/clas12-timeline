    ...
    (1..number_of_wires_this_layer).collect{wire_number->
      ...
      if(h1!=null) {
        if (h1.getBinContent(h1.getMaximumBin()) > 25 && h1.getEntries()>100){
          ...
          f1.setParameter(0, 1);
          data[run].put(String.format("fit_ahdc_residual_LR_layer%d_wire_number%02d", layer_number, wire_number), f1)
          data[run].put(String.format("mean_ahdc_residual_LR_layer%d_wire_number%02d", layer_number, wire_number), -2.0 + h1_mean)
          data[run].put(String.format("width_ahdc_residual_LR_layer%d_wire_number%02d", layer_number, wire_number), -2.0 + h1_rms)
          has_data.set(true)
        }
      }
    }
