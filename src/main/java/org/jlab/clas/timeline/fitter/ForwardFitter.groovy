...
*
* Writer: Sangbaek Lee, Andrey Kim, Mohammad Hattawy
*
**/
...
class ForwardFitter{
  ...
  static F1D fitRGDbimodal(H1F h1, float mean1, float mean2, float sigma1, float sigma2, float range_min, float range_max) {
  ...
  static F1D fit(H1F h1) {
    ...
    double hMean = h1.getAxis().getBinCenter(h1.getMaximumBin());
    double hRMS  = h1.getRMS(); // cm
    f1.setRange(hMean - 3*hRMS, hMean + 3*hRMS);
    f1.setParameter(0, hAmp);
    f1.setParameter(1, hMean);
    f1.setParameter(2, hRMS);
    MoreFitter.fit(f1,h1,"LQ");
