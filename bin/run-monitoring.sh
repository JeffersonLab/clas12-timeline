...
for rdir in ${rdirs[@]}; do
  ...
  if [ -z "$runnum" ] || ${modes['swifjob']}; then # otherwise, use RUN::config from a HIPO file (NOTE: assumes all HIPO files have the same run number)
    if ${modes['flatdir']}; then
      ...
      runnum=$($TIMELINESRC/bin/run-groovy-timeline.sh $TIMELINE_GROOVY_OPTS $TIMELINESRC/bin/get-run-number.groovy $rdir | tail -n1 | grep -m1 -o -E "[0-9]+" || echo '')
    else
      ...
      runnum=$($TIMELINESRC/bin/run-groovy-timeline.sh $TIMELINE_GROOVY_OPTS $TIMELINESRC/bin/get-run-number.groovy $firstHipo | tail -n1 | grep -m1 -o -E "[0-9]+" || echo '')
    fi
  fi
