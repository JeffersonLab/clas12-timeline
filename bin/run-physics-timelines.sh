...
exe $TIMELINESRC/bin/run-groovy-timeline.sh $TIMELINE_GROOVY_OPTS buildChargeTree.groovy $qaDir

# loop over datasets
# trigger electrons monitor
exe $TIMELINESRC/bin/run-groovy-timeline.sh $TIMELINE_GROOVY_OPTS qaPlot.groovy $qaDir
exe $TIMELINESRC/bin/run-groovy-timeline.sh $TIMELINE_GROOVY_OPTS qaCut.groovy $qaDir $dataset
# FT electrons
exe $TIMELINESRC/bin/run-groovy-timeline.sh $TIMELINE_GROOVY_OPTS qaPlot.groovy $qaDir FT
exe $TIMELINESRC/bin/run-groovy-timeline.sh $TIMELINE_GROOVY_OPTS qaCut.groovy $qaDir $dataset FT
# meld FT and FD JSON files
exe $TIMELINESRC/bin/run-groovy-timeline.sh $TIMELINE_GROOVY_OPTS mergeFTandFD.groovy $qaDir
# general monitor
exe $TIMELINESRC/bin/run-groovy-timeline.sh $TIMELINE_GROOVY_OPTS monitorPlot.groovy $qaDir
# move timelines to output area
exe ./stageTimelines.sh $qaDir $finalDir
# trash empty files
exe $TIMELINESRC/bin/run-groovy-timeline.sh $TIMELINE_GROOVY_OPTS $TIMELINESRC/qa-physics/removeEmptyFiles.groovy $outputDir/trash $finalDir
