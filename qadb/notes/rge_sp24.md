...
> [!NOTE]
> Using `--dstdir` and `--fast-ls` here rather than `--skimdir`, since we're using a directory of DST files.

check cache:
```bash
bin/qtl histogram -d rge_sp24 --check-cache --dstdir --fast-ls --focus-physics $(ls -d /cache/clas12/rg-e/production/spring2024/pass1/*/*/dst/recon)
```
