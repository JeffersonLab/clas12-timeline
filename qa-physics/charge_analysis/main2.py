...
def main():
    hipo_dir = "/cache/clas12/rg-a/production/recon/spring2018/10.59gev/torus-1/pass1/train/DVCSWagon/"
    hipo_dir = "/w/hallb-scshelf2102/clas12/singh/Softwares/QADB_studies/python/test_rgk/"
    hipo_dir = "/w/hallb-scshelf2102/clas12/singh/Softwares/coatjava_rollover2/inbending_rga_test/"
    hipo_dir = "/lustre24/expphy/cache/clas12/rg-a/production/recon/spring2018/10.59gev/torus-1/pass1/dst/train/nSidis/"
    #hipo_dir = "/w/hallb-scshelf2102/clas12/singh/Softwares/coatjava/test_rollover/"
    #hipo_dir = "/w/hallb-scshelf2102/clas12/singh/Softwares/coatjava_rollover2/test_rollover2/"
    hipo_files = sorted(glob(os.path.join(hipo_dir, "*.hipo")))
    hipo_files = hipo_files[:3]

    if not hipo_files:
        logger.error(f"No .hipo files found in {hipo_dir}")
        return
    
    logger.info(f"Found {len(hipo_files)} .hipo files in {hipo_dir}")

    for hipo_file in hipo_files:
        logger.info(f"Processing file: {hipo_file}")

        reader = hipolib.hreader("./../../hipo/install/lib/")
