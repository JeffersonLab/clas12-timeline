...
for bit in ${defect_bits_FD[@]}; do
  ...
  $TIMELINESRC/bin/run-groovy-timeline.sh $TIMELINE_GROOVY_OPTS qaCut.groovy $tlDir $dataset false $bit
...
for bit in ${defect_bits_FT[@]}; do
  ...
  $TIMELINESRC/bin/run-groovy-timeline.sh $TIMELINE_GROOVY_OPTS qaCut.groovy $tlDir $dataset FT $bit
