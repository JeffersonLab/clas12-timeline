...
* `../bin/run-monitoring.sh`: runs `monitorRead.groovy` on DSTs using `slurm`
  * **IMPORTANT**: call this first with the `--check-cache` option to make sure that ALL required DST files are pinned; if all files are on `/cache`, you may proceed, removing the `--check-cache` option
